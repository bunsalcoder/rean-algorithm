...
import {
  ...
  buildInsertionSortFrames,
  generateRandomArray,
  parseNumberArray,
  type InsertionSortFrame,
...
function getCellState(index: number, frame: InsertionSortFrame): CellState {
  if (frame.phase === 'done') {
    return 'completed'
  }
  ...
  if (
    ...
    index === frame.insertIndex &&
    (frame.phase === 'insert' || frame.phase === 'no-shift')
  ) {
    return 'inserted'
  }

  if (frame.shiftingIndex !== null && index === frame.shiftingIndex) {
    return 'shifting'
  }

  if (
    frame.shiftingIndex !== null &&
    index === frame.shiftingIndex + 1 &&
    frame.phase === 'shift'
  ) {
    return 'shifting'
  }

  if (frame.comparingIndex !== null && index === frame.comparingIndex) {
    return 'comparing'
  }
  ...
  if (
    ...
    frame.keyValue !== null &&
    frame.array[index] === frame.keyValue &&
    (frame.phase === 'select-key' ||
      frame.phase === 'compare' ||
      frame.phase === 'shift' ||
      frame.phase === 'no-shift')
  ) {
    return 'key'
  }
...
const cellStateClasses: Record<CellState, string> = {
  ...
  key: 'border-sky-500 bg-sky-500/15 text-foreground shadow-sm ring-2 ring-sky-500/30 dark:border-sky-400 dark:bg-sky-400/15',
  comparing:
  ...
  shifting:
    'border-rose-500 bg-rose-500/15 text-foreground shadow-sm ring-2 ring-rose-500/30 dark:border-rose-400 dark:bg-rose-400/15 translate-x-1',
  inserted:
    'border-violet-500 bg-violet-500/15 text-foreground shadow-sm ring-2 ring-violet-500/30 dark:border-violet-400 dark:bg-violet-400/15 -translate-y-1',
  sorted:
...
function getPointerLabel(index: number, frame: InsertionSortFrame): string | null {
  if (
    frame.insertIndex === index &&
    (frame.phase === 'insert' || frame.phase === 'no-shift')
  ) {
    return 'insert'
  }

  if (frame.shiftingIndex === index && frame.phase === 'shift') {
    return 'shift'
  }

  if (frame.comparingIndex === index) {
    return 'compare'
  }

  if (
    frame.keyIndex === index &&
    frame.keyValue !== null &&
    frame.array[index] === frame.keyValue &&
    frame.phase !== 'pass-complete' &&
    frame.phase !== 'insert'
...
export function InsertionSortVisualization() {
  const [arrayInput, setArrayInput] = useState(
    formatList(INSERTION_SORT_DEFAULT_ARRAY),
  )
  const [array, setArray] = useState<number[]>([...INSERTION_SORT_DEFAULT_ARRAY])
  const [inputError, setInputError] = useState<string | null>(null)
  const [stepIndex, setStepIndex] = useState(0)
  const [isPlaying, setIsPlaying] = useState(false)

  const frames = useMemo(() => buildInsertionSortFrames(array), [array])
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <label className="min-w-0">
          ...
          <input
            ...
            aria-describedby={
              inputError ? 'insertion-sort-array-error' : undefined
            }
          />
        </label>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {[
          ...
          },
          {
            key: 'key-index',
            label: 'Key Index',
            value: frame.keyIndex !== null ? String(frame.keyIndex) : '—',
          },
          {
            key: 'key-value',
            label: 'Key Value',
            value: frame.keyValue !== null ? String(frame.keyValue) : '—',
          },
          {
            key: 'comparing',
            label: 'Comparing Index',
            value:
              frame.comparingIndex !== null
                ? String(frame.comparingIndex)
                : '—',
          },
          {
            key: 'comparisons',
          ...
          },
          {
            key: 'shifts',
            label: 'Shifts',
            value: String(frame.totalShifts),
          },
        ].map((stat) => (
          <div
            key={stat.key}
      ...
      {frame.keyValue !== null && frame.phase !== 'done' ? (
        <div className="rounded-lg border border-sky-500/30 bg-sky-500/5 px-4 py-3 dark:bg-sky-400/10">
          ...
          <p className="mt-1 font-mono text-base font-medium text-foreground">
            {frame.keyValue}
            {frame.keyIndex !== null ? ` from index ${frame.keyIndex}` : ''}
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            The key is the element we are currently trying to place into the
            correct position.
          </p>
        </div>
      ) : null}
      ...
      >
        ...
        <div className="mx-auto flex w-max min-w-full items-end justify-center gap-1 sm:gap-2">
          {frame.array.map((value, index) => {
            ...
            return (
              <div
                key={`${index}-${value}-${frame.phase}-${frame.totalShifts}`}
      ...
      <div
        className={cn(
          ...
          (frame.phase === 'insert' || frame.phase === 'pass-complete') &&
